...
def execute(filters=None):
    ...
    if filters:
        if filters.get("status") == "Pending":
            conditions.append("ACF.it_return_status = 'Pending'")
        elif filters.get("status") == "Completed Before Due Date":
            conditions.append("ACF.it_return_status = 'Completed' AND ACF.it_return_submitted_on <= ACF.it_return_due_date")
    ...
    if conditions:
        sql_query = f"""
            SELECT
                ...
                ACF.it_return_due_date AS due_date,
                ACF.it_return_status AS status
            FROM
