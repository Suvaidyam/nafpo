...
def execute(filters=None):
    ...
    if filters:
        if filters.get("status") == "Pending":
            conditions.append("ACF.mgt_7_status = 'Pending'")
        elif filters.get("status") == "Completed Before Due Date":
            conditions.append("ACF.mgt_7_status = 'Completed' AND ACF.mgt_7_submitted_on <= ACF.mgt_7_due_date")
    ...
    if conditions:
        sql_query = f"""
            SELECT
                ...
                ACF.mgt_7_due_date AS due_date,
                ACF.mgt_7_status AS status
            FROM
