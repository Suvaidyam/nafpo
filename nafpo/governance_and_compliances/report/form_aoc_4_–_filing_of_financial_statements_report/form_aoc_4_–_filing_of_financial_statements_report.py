...
def execute(filters=None):
    ...
    if filters:
        if filters.get("status") == "Pending":
            conditions.append("ACF.aoc_4_status = 'Pending'")
        elif filters.get("status") == "Completed Before Due Date":
            conditions.append("ACF.aoc_4_status = 'Completed' AND ACF.aoc_4_submitted_on <= ACF.aoc_4_due_date")
    ...
    if conditions:
        sql_query = f"""
            SELECT
                ...
                ACF.aoc_4_due_date AS due_date,
                ACF.aoc_4_status AS status
            FROM
